...
class ConvNet_MaxPool_Goal(nn.Module):
    def __init__(self, dropout):
        """
        ...
        # where alpha and beta are learned parameters, which allow the model to
        # learn the best scaling for the data.
        self.sensors_norm = nn.BatchNorm1d(num_features=4)
        self.goal_norm = nn.BatchNorm1d(num_features=3)
        ...
        self.fc1 = nn.Linear(15 * 96 + 3, 128)
        self.drop1 = nn.Dropout(dropout) if dropout > 0 else nn.Identity()
        self.fc2 = nn.Linear(128, 128)
        self.drop2 = nn.Dropout(dropout) if dropout > 0 else nn.Identity()
        self.fc3 = nn.Linear(128, 2)

    def forward(self, sensors, goals):
        """

        :param sensors: input tensor with shape samples ⨉ channels ⨉ angles
        :param goals: input tensor with shape samples ⨉ {x, y, theta}
        :return: output tensor with shape samples ⨉ wheels
        """
        sensors = self.sensors_norm(sensors)
        goals = self.goal_norm(goals)

        net = F.relu(self.conv1(sensors))
        net = F.relu(self.conv2(net))
